...
public class Arena {
    ...
    public void removeObstacle(int x, int y) {
        for(Obstacle obstacle : obstacles){
            if(obstacle.getCoordinate().getX() == x && obstacle.getCoordinate().getY() == y){
                //Obstacle.removeObstacleById(obstacle.getId()+1);
    ...
    public void clearObstacles(){
        //for (int i = obstacles.size(); i > 0; i--) {
            //Obstacle obstacle = obstacles.getObstacleById(i);
            //obstacles.removeObstacleById(obstacle.getId());
        //}

        for (Obstacle obstacle : obstacles) {
            Obstacle.removeObstacleById(obstacle.getId());
        }
